...
function subscriberEmail() {
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td align="center">
    <table role="presentation" width="520" cellpadding="0" cellspacing="0" style="max-width:520px">
      <tr><td style="font:700 13px 'Courier New',monospace;letter-spacing:.28em;text-transform:uppercase;color:#d4af37;padding-bottom:18px">Benji's AI Empire</td></tr>
      ...
      <tr><td style="padding-top:30px;border-top:1px solid rgba(244,236,216,.14);margin-top:24px;font-size:12px;color:rgba(244,236,216,.4)">
        — Benji &middot; benjisaiempire.com
      </td></tr>
    </table>
...
export async function POST(req: Request) {
  ...
  try {
    // 1) Deliver the plans to the subscriber.
  ...
  // 2) Notify Ben of the new lead (his capture). Best-effort — never block the user.
  if (NOTIFY.length) {
    try {
      await resend.emails.send({
        from: FROM,
        ...
        html: `<p>New email captured on the bridge page:</p><p style="font-size:18px"><strong>${email}</strong></p><p style="color:#888">${new Date().toISOString()}</p>`,
      });
    } catch {
      // ignore — the subscriber already got their plans
    }
  }
